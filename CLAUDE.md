...
- **Setting — `cloudSyncEnabled`** (default `true`). Surfaced as a per-account toggle in Settings → System → Cloud sync, plus a one-tap "Resume sync" pill in the app shell (sub-route header + cockpit corner) when it's been turned off. When `false`, the sync engine pauses outbound writes for that account and the cockpit stays local-first. Flipping back to `true` doesn't backfill historical data — sync is new-only.
...
- **5 June 2026 — Stay-logged-in pass for the iPad PWA.** Firebase Auth on web defaults to `browserLocalPersistence` (localStorage), which iPad Safari purges under ITP after the ~7-day cap — meaning the home-screen icon dropped the user at `/login` even mid-grace-window. Five-piece fix: (1) `getFirebaseAuth()` goes through `initializeAuth(app, { persistence: [indexedDBLocalPersistence, browserLocalPersistence] })` so the session lives in IDB, with localStorage as the fallback if the runtime refuses IDB; (2) PWA manifest `start_url` flips from `/` to `/app` so the home-screen icon lands on the cockpit's existing loading gate (which redirects to `/login` on a stale session and renders the cockpit on a live one); (3) `useSession()` force-refreshes the ID token on `visibilitychange` (visible) and `pageshow` so a long-suspended PWA doesn't surprise the next `/api/admin/*` call with a 401 race; (4) a 50-min interval also refreshes the token while the tab is open so it never expires under us; (5) `navigator.storage.persist()` is requested on app mount to ask the browser to treat the IDB store as persistent (Safari's heuristic gates this but the request doesn't hurt). The cockpit's "Resume sync" pill (added 4 June below) keeps the original `cloudSyncEnabled` toggle from being a foot-gun: tap once to flip back on without diving into Settings.
...
- Cockpit data (conversations, turns, voiceprints, voice samples, memories, settings, etc.) is Dexie/IndexedDB-first and syncs to per-user Firebase subtrees by default via the write-behind engine in `src/lib/sync/`. Dexie stays the live source of truth on the device — don't reverse that, don't make the cockpit wait on network round-trips, and don't add a new table without giving the sync engine a path for it. The per-account `cloudSyncEnabled` toggle is the user's escape hatch back to local-only; respect it.
