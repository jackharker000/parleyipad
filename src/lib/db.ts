...
class AacDb extends Dexie {
  ...
  constructor() {
    ...
    // Tier 2 (post-conversation analysis). Bumps the schema by adding:
    //   * profile_proposals — per-person profile-update review queue
    //   * segment_mfccs    — per-utterance MFCC vectors for re-diarization
    // Also extends the existing `people` and `voiceprints` stores with new
    // queryable indexes (`status`, `confidence`).
    //
    // NOTE FOR MERGE: this worktree branched off a tree that only had v6
    // declared, so v7 (voiceprint_contributions) lands here too. If the
    // orchestrator's other tier merges in a different ordering, renumber
    // so the v7→v8 sequence stays contiguous.
