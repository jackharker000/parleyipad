...
/**
 * The cockpit's right-side speaker roster panel. Replaces the older
 * SpeakerColumn (which showed only the top-1 candidate and a flat alt list)
 * with the legacy SpeakerPanel's richer roster view: one row per person in
 * the active conversation, each with a named confidence band, a per-row
 * Confirm / Reject action when borderline, and a "hasn't spoken yet"
 * status for declared participants who haven't been heard yet.
 *
      ...
      <div className="space-y-2 p-4">
        {rows.length === 0 ? (
          <p className="text-sm text-muted-foreground">
            {isLive ? "Listening…" : "Pick a roster and tap Record."}
          </p>
        ) : (
          rows.map((row) => (
            <SpeakerRow
